# Описани
 ...
 Подключаемся по nc, и видим привер, который нужно решить.  Если пытаемся решить, получаем надпись `Wrong! Starting over...` . Поэтому попытаемся сделать эти действия быстрее используя `python`
